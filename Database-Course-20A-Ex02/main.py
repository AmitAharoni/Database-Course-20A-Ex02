...
def splitANDCond(toCheck):
    while (isSimple_CondValid(toCheck) or isCondANDcondValid(toCheck) or isCondORcondValid(toCheck) or isPartCONDValid(toCheck)) and isPartCONDValid(toCheck):
        toCheck = toCheck[1:toCheck.__len__()-1]
    if (isSimple_CondValid(toCheck) or isCondANDcondValid(toCheck) or isCondORcondValid(toCheck) or isPartCONDValid(toCheck)) and isCondANDcondValid(toCheck):
            ...
            if(indexOfPi+1 < operatorList.__len__()):
                if isinstance(operatorList[indexOfPi + 1], Sigma):
                    ...
                    if tableOfSigma is not None:
                        if attributePi.__contains__(tableOfSigma) and notContainTheOtherTable(attributePi, tableOfSigma):
...
def splitANDorORCond(condToSplit):
    ...
    if (isSimple_CondValid(condToSplit) or isCondANDcondValid(condToSplit) or isCondORcondValid(condToSplit) or isPartCONDValid(condToSplit)) and isCondANDcondValid(condToSplit):
    ...
    if (isSimple_CondValid(condToSplit) or isCondANDcondValid(condToSplit) or isCondORcondValid(condToSplit) or isPartCONDValid(
